...
function set_follow_Status_2_str($lists){
         $status = $lists['$status'];
         $returnStr = '无状态';
...
function add_tongji_item($dataAry = null, $bgcolor, $bgsolid, $icon, $data, $tetil, $link){
         if(is_null($dataAry))return false;
         $temAry = array(
             "bgcolor" => $bgcolor,
             "bgsolid" => $bgsolid,
             "icon" => $icon,
...
function get_category_array($id=0){
    $arr = array();
    $map ['token'] = get_token ();
    $map ['pid'] = $id;
    $resultData = M("shop_category")->where($map)->field(array('id','name','name' => 'text','pid','state'))->select();

    if($resultData && count($resultData) > 0){
        $temp = 0;
        while($temp < count($resultData)){ //循环记录集
            $rows = $resultData[$temp];
            $rows['_parentId'] = intval($rows['pid']);
            $rows['isCatalog'] = true;
            $childrens = get_category_array($rows['id']); //调用函数，传入参数，继续查询下级
            if($childrens){
                $rows['children'] = $childrens;
            }else{
                unset( $rows['children']);
            }
            $arr[] = $rows; //组合数组
...
function get_like_stars($likenum = 0){
    $retStr = '☆☆☆☆☆';
    switch($likenum){
        case 0:$retStr = '☆☆☆☆☆'; break;
        case 1:$retStr = '★☆☆☆☆'; break;
...
function get_category_goods_ary($id = 0){
         $rtAry = getCGARY($id);
         $map ['token'] = get_token ();
         $map ['cate_id'] = 0;
...
function getCGARY($id){
    $arr = array();
    $categoryAry = get_category_array($id);

    if($categoryAry && count($categoryAry) > 0){
        $temp = 0;
        while($temp < count($categoryAry)) { //循环记录集
            $nodes = $categoryAry[$temp];
            if( $nodes.isCatalog ){
                $map ['token'] = get_token ();
                ...
                if(count($goods) > 0 || count($nodes['children']) > 0){
                    ...
                    if( count($goods) > 0){
                        $childrens =  $nodes['children'] ;
                        if(count($childrens ) > 0){
                            $nodes['children'] = array_merge($childrens,$goods);
                        }else{
                            $nodes['children']  = $goods;
                        }
                ...
                if(! $nodes.isCatalog) $nodes['children'] = getCGARY($nodes['id']);
            }
...
function get_category_goods_json($id = 0){
